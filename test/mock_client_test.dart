...
void main() {
  test('handles a request', () async {
    var client = MockClient((request) async => http.Response(json.encode(request.bodyFields), 200, request: request, headers: {'content-type': 'application/json'}));

    var response = await client.post(Uri.http('example.com', '/foo'), body: {'field1': 'value1', 'field2': 'value2'});
    expect(response.body, parse(equals({'field1': 'value1', 'field2': 'value2'})));
  });
  ...
  test('handles a request with no body', () async {
    ...
    expect(await client.read(Uri.http('example.com', '/foo')), equals('you did it'));
  });
}
